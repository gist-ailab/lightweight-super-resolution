...
gpus_list=range(opt.gpus, int(opt.gpus)+1)
print(opt)
...
def eval():
    ...
    for batch in testing_data_loader:
        ...
        save_img(prediction.cpu().data, str(count), 'pred')
        save_img(target, str(count), 'target')
        save_img(input.cpu().data, str(count), 'input')
...
def save_img(img, img_name, pred_flag):
    ...
    elif pred_flag == 'target':
        save_fn = save_dir +'/'+ img_name+'.png'
    elif pred_flag == 'input':
        save_fn = save_dir + '/' + img_name+'_input.png'
